...
export default function BecomeSellerPending() {
  ...
  useEffect(() => {
    const fetchUserRequests = async () => {
      try {
        const response = await axios.get<{ recordsets: SellerRequest[][]}>('http://localhost:6969/api/users/become-seller-requests');
  ...
  const handleAccept = async (id: string) => {
    try {
      await axios.put(`http://localhost:6969/api/users/become-seller-requests/${id}`);
  ...
  const handleDecline = async (id: string) => {
    ...
    try {
      await axios.put(`http://localhost:6969/api/users/decline-become-seller-requests/${id}`);
      setRequests(requests.map(request => 
